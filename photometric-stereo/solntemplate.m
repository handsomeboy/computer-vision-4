...
if isunix
  ...
  codeDir = 'C:\Users\Tan\Documents\MATLAB\A1handout\';
  dataDir = [codeDir, 'ppmImages\'];
  chromeDir = [dataDir, 'chrome\'];
end
cd(codeDir)
...
for useImageSet = 3 % 1:6  or just choose one image set, e.g. 3
  ...
  figure(8); clf; showIm(reshape(rmsErr, imsize));
...
%   continue;
  ...
  for m=1:3
    for j=1:nDir
        ...
        RGBim = ppmRead(fname);
        imColor = RGBim(:,:,m);
        imDataColor(:,j) = imColor(:);
    end
    imDataColorCrop = imDataColor(mask,1:nDirChrome);
    [temp, albedoColorCrop] = fitReflectance(imDataColorCrop,Lchrome);
    
    albedoTemp = zeros(numPixels,1);
    albedoTemp(mask) = albedoColorCrop;
    albedo(:,m) = albedoTemp;
  end
